...
function formatLocalTimestamp(year, month, day, hh, min, ss) {
  return `${String(year).padStart(4, "0")}-${String(month).padStart(2, "0")}-${String(day).padStart(
    2,
    "0",
  )}T${String(hh).padStart(2, "0")}:${String(min).padStart(2, "0")}:${String(ss).padStart(2, "0")}${ARGENTINA_OFFSET}`;
}
...
function extractLocation(restText) {
  ...
  const shortMatch =
    locationRaw.match(/\b(RP\s*\d+|RN\s*\d+)\b/i) || locationRaw.match(/\b(DESCONOCIDO)\b/i);
...
function parseExcesos(bodyText) {
  ...
  return normalizeBodyLines(bodyText)
    .map((line) => {
      ...
      const plateMatch = line.match(PLATE_REGEX);
      if (!plateMatch) return null;
      const plateRaw = plateMatch[1];
...
function parseNoIdentificados(bodyText) {
  ...
  return normalizeBodyLines(bodyText)
    .map((line) => {
      ...
      const plateMatch = beforeReason.match(PLATE_REGEX);
      if (!plateMatch) return null;
      const plateRaw = plateMatch[1];
...
function parseContactoSinIdentificacion(bodyText) {
  ...
  const regex =
    /^(.*?)\s{2,}(.*?)\s{2,}([A-Z0-9\-\s]+?)\s{2,}(\d{2}-\d{2}-\d{4})\s+(\d{2}:\d{2}:\d{2})$/i;
